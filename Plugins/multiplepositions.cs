...
namespace MissionPlanner.plugins
{
    public class MultiplePositions : Plugin.Plugin
    {
        ...
        {
            rootbut = new ToolStripMenuItem("Show other positions");
                ...
                case MAVLink.MAVLINK_MSG_ID.GPS_RAW_INT:
                    {
                        ...
                        UpdateOrCreate(new PointLatLng(pos.lat / 1e7, pos.lon / 1e7), Host.cs.yaw / 100.0f, pos.cog / 100.0f, ID, "GPS1", 2);
                        break;
                    }
                case MAVLink.MAVLINK_MSG_ID.GPS2_RAW:
                    {
                        var pos = (MAVLink.mavlink_gps2_raw_t)message.data;
                        UpdateOrCreate(new PointLatLng(pos.lat / 1e7, pos.lon / 1e7), Host.cs.yaw / 100.0f, pos.cog / 100.0f, ID, "GPS2", 2);
                        break;
                    }
                case MAVLink.MAVLINK_MSG_ID.AHRS2:
                    {
                        var pos = (MAVLink.mavlink_ahrs2_t)message.data;
                        UpdateOrCreate(new PointLatLng(pos.lat / 1e7, pos.lng / 1e7), pos.yaw * 180 / 3.1415f, Host.cs.groundcourse, ID, "AHRS2", 5);
                        break;
                    }
        ...
        private void UpdateOrCreate(PointLatLng pointLatLng, float heading, float cog, int ID, string sourcetext = "", int color = 1)
        {
            ...
            if (existing.Count() > 0)
            {
                ...
                ((GMapMarkerPlane)existing.First()).Target = float.NaN;
                ((GMapMarkerPlane)existing.First()).Nav_bearing = float.NaN;
                ((GMapMarkerPlane_edit)existing.First()).LastUpdate = DateTime.Now;
            }
            else
            {
                var marker = new GMapMarkerPlane_edit(pointLatLng, heading, color)
    ...
    public class GMapMarkerPlane_edit : GMapMarkerPlane
    {
        ...
        public GMapMarkerPlane_edit(PointLatLng p, float heading, int color) : base(color, p, heading, heading, heading, heading, -1)
        {
